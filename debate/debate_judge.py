...
class DebateJudge(GreenAgent):
    def __init__(self):
        ...
        self.llm_provider = os.getenv("JUDGE_LLM_PROVIDER", "gemini").lower()
        
        if self.llm_provider == "gemini":
            self._client = genai.Client()
            self._model = os.getenv("GEMINI_MODEL", "gemini-2.5-flash")
        elif self.llm_provider == "ollama":
            ollama_model = os.getenv("JUDGE_OLLAMA_MODEL", "llama3.1:8b")
            self._model = f"ollama_chat/{ollama_model}"
        else:
    ...
    async def judge_debate(self, topic: str, debate_text: str) -> DebateEval:
        ...
        Please output the result as a JSON object with the following structure:
        {
            "pro_debater": {
                "emotional_appeal": <score>,
                "argument_clarity": <score>,
                "argument_arrangement": <score>,
            ...
            "winner": "<pro_debater or con_debater>",
        ...
        if self.llm_provider == "gemini":
            ...
            # Validate and create DebateEval object
            return DebateEval(**response_json)
